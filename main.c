    ...
    if ( pSpriteTourSol )  //si le permier sprite a bien �t� charg�, on suppose que les autres aussi
    {
        ...
        while ( cont != 0 ){   //VOUS DEVEZ GERER (DETECTER) LA FIN DU JEU -> tourRoiDetruite
                ...
                if(jeu[tabParcours[0][0]][tabParcours[0][1]] == NULL){
                        int type = rand() % 4;
                        switch(type){
                        case 0: AjouterUnite(&listeHorde, creeDragon(tabParcours[0][0], tabParcours[0][1])); break;
                        case 1: AjouterUnite(&listeHorde, creeArcher(tabParcours[0][0], tabParcours[0][1])); break;
                        case 2: AjouterUnite(&listeHorde, creeGargouille(tabParcours[0][0], tabParcours[0][1])); break;
                        case 3: AjouterUnite(&listeHorde, creeChevalier(tabParcours[0][0], tabParcours[0][1])); break;
                                }
                        }
                }
                ...
                if((rand() % 100) < 20 && compteur_tour % 5 == 0){
